...
const ENGLISH_TEXT = {
  ...
  export: {
    ...
    helper:
      'URLs encode the model configuration, plot options, overlay choice, and cross-section controls. CSV exports include all sampled state variables for the current settings.',
...
export const TRANSLATIONS: Record<Language, UiText> = {
  ...
  ja: {
    ...
    export: {
      ...
      helper:
        'URLにはモデル設定、プロットオプション、オーバーレイ選択、断面表示設定が含まれます。CSVには現在の設定でサンプリングされた全状態変数が含まれます。',
  ...
  es: {
    ...
    export: {
      ...
      helper:
        'Las URL codifican la configuración del modelo, opciones de gráfica, selección de superposición y controles de sección. Los CSV incluyen todas las variables de estado muestreadas para la configuración actual.',
